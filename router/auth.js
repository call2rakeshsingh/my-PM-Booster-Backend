...
router.post('/login', async (req, res) => {
    ...
    const userInfo = [userData.userName, userData.branchName, userData.userEmail]
...
router.get('/validateuser', addEmailInHelpticket, async (req, res) => {
  try {
    ...
    res.status(200).json([userName, branchName, userEmail]);
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Something went wrong' });
  }
